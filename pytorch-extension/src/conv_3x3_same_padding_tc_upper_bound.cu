...
__global__ void conv_cuda_tc_forward_kernel_upper_bound(
  ...
  for (int i = 0; i < input.size(3) * UNROLLED_KERNEL_SIZE; i += WMMA_K) {
    ...
    // if (aRow < GEMM_M && aCol < GEMM_K && bRow < GEMM_K && bCol < GEMM_N) {
        ...
        wmma::load_matrix_sync(a_frag, smem_input_tile, WMMA_K);
        wmma::load_matrix_sync(b_frag, smem_weight_tile, WMMA_N);
  ...
  // if (cRow < GEMM_M && cCol < GEMM_N) {
    ...
    wmma::store_matrix_sync(static_cast<__half *>(output_ptr), acc_frag, WMMA_M, wmma::mem_row_major);
...
//   int GEMM_K = C * R * S;
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(torch::kFloat16, "conv_forward_tc_cuda_upper_bound", ([&] {
    conv_cuda_tc_forward_kernel_upper_bound<<<gridDim, blockDim>>>(
